...
import { companyIdentityForName } from "@/lib/company-directory";
...
const worldCountries = countryCollection.features
  .map((country) => ({
    ...
    bounds: mapPath.bounds(country) as [MapPoint, MapPoint],
  }))
...
export function GlobalMapView({
  ...
  const previewShape = previewCountry
    ...
    : null;
  const previewPlacement = mapCalloutPlacement({
    bounds: previewShape?.bounds ?? null,
  ...
  const { left: previewLeft, top: previewTop, side: previewSide, vertical: previewVertical } = previewPlacement;
    ...
    <div className="map-layout atlas-layout">
        ...
        >
          ...
          {previewCountry ? (
            ...
            >
              ...
              {previewResearch ? (
                <div className="map-callout-companies" aria-label={`Largest listed companies in ${previewCountry.name}`}>
                  {previewResearch.companies.slice(0, 4).map((company) => {
                    const identity = companyIdentityForName(company);
                    return identity ? <CompanyLink key={company} identity={identity} compact /> : null;
                  })}
                </div>
              ) : null}
      ...
      {selectedCountry ? (
        <section className="country-dashboard">
            ...
            <div className="country-research-grid">
              <article>
                ...
                <div className="company-list">
                  {selectedResearch.companies.map((company) => {
                    const identity = companyIdentityForName(company);
                    return identity ? <CompanyLink key={company} identity={identity} /> : <span key={company}>{company}</span>;
                  })}
                </div>
              </article>
              <article>
          ...
          {selectedResearch && selectedAnalytics && selectedValuation ? (
            <CountryInsights
...
function CountryInsights({
  ...
  valuation: ReturnType<typeof valuationMetrics>;
}) {
  const valuationSignal = analytics.pe < 15
    ? "The broad market screens at a relatively low earnings multiple."
    : analytics.pe > 25
      ? "The broad market carries a high earnings multiple and depends more heavily on future growth."
      : "The broad market sits in a middle valuation range rather than at an obvious extreme.";
  const balanceSheetSignal = analytics.debtToGdp > 100
    ? "High sovereign debt can reduce fiscal flexibility when growth slows or funding costs rise."
    : analytics.debtToGdp < 55
      ? "Lower public debt gives policymakers comparatively more room to respond to a downturn."
      : "Public debt is meaningful but not the market's only macro sensitivity.";
  const qualitySignal = analytics.sharpe >= 0.9 && analytics.solvency >= 70
    ? "Risk-adjusted returns and solvency screen as comparatively resilient."
    : analytics.sharpe < 0.55 || analytics.solvency < 55
      ? "The current screen points to weaker risk-adjusted returns or financial resilience."
      : "Risk-adjusted returns and solvency sit in a mixed middle range.";

  return (
    <section className="country-insights" aria-labelledby="country-insights-title">
      <header>
        <div>
          <span className="eyebrow">Country insights</span>
          <h3 id="country-insights-title">What the {country.name} setup is saying</h3>
        </div>
        <span>Contextual research · not a trade recommendation</span>
      </header>
      <div className="country-insight-grid">
        <article className="valuation">
          <span>01 · Valuation and growth</span>
          <h4>{analytics.pe.toFixed(1)}x earnings · {analytics.earningsGrowth.toFixed(1)}% growth</h4>
          <dl>
            <div><dt>Signal</dt><dd>{valuationSignal}</dd></div>
            <div><dt>Why it matters</dt><dd>PEG is {valuation.peg === null ? "not meaningful with the available growth input" : `${valuation.peg.toFixed(2)}`}; free-cash-flow yield is {analytics.fcfYield.toFixed(1)}%.</dd></div>
            <div><dt>Monitor</dt><dd>Earnings revisions, index concentration, and whether growth is broadening beyond the largest constituents.</dd></div>
          </dl>
        </article>
        <article className="resilience">
          <span>02 · Resilience</span>
          <h4>Sharpe {analytics.sharpe.toFixed(2)} · solvency {analytics.solvency}/100</h4>
          <dl>
            <div><dt>Signal</dt><dd>{qualitySignal}</dd></div>
            <div><dt>Why it matters</dt><dd>{balanceSheetSignal}</dd></div>
            <div><dt>Monitor</dt><dd>{country.policyRate.value}, credit conditions, the {country.currency}, and refinancing pressure.</dd></div>
          </dl>
        </article>
        <article className="structure">
          <span>03 · Market structure</span>
          <h4>{country.keyIndex}</h4>
          <dl>
            <div><dt>Signal</dt><dd>{country.note}</dd></div>
            <div><dt>Why it matters</dt><dd>{country.sectors}. {research.materials}</dd></div>
            <div><dt>Monitor</dt><dd>{research.companies.slice(0, 4).join(", ")} and changes in sector leadership.</dd></div>
          </dl>
        </article>
        <article className="watch">
          <span>04 · Forward watch</span>
          <h4>Policy, politics, and catalysts</h4>
          <dl>
            <div><dt>Signal</dt><dd>{research.policyWatch}</dd></div>
            <div><dt>Why it matters</dt><dd>{research.takeaways[0]}</dd></div>
            <div><dt>Monitor</dt><dd>{research.takeaways.slice(1).join(" ") || "Recheck the thesis as new macro and company data arrives."}</dd></div>
          </dl>
        </article>
      </div>
      <footer>
        Built from the market profile above. Daily, event-driven, and annual observations retain their original cadence and source labels.
      </footer>
    </section>
  );
}
